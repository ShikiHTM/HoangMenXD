    ...
    if (args[0] == "beatmap") {
      ...
      osuapi.getBeatmaps({ b: `${beatmapID}` }).then((map) => {
        ...
        const embed = new discord.MessageEmbed()
          ...
          .setAuthor(
            `${beatmap.artist} - ${beatmap.title} by ${beatmap.creator}`,
          ...
          .setDescription(
            `▸**Length:** ${
              Math.floor(beatmap.length.total / 60) +
              ":" +
              (beatmap.length.total % 60 < 10
                ? "0" + (beatmap.length.total % 60)
                : beatmap.length.total % 60)
            } ▸**BPM:** ${
              beatmap.bpm
            }\n**Download:** [video](https://osu.ppy.sh/beatmapsets/${
              beatmap.id
            }/download) **|** [No video](https://osu.ppy.sh/beatmapsets/${
              beatmap.id
            }/download?noVideo=1) **|** [chimu](https://api.chimu.moe/v1/download/${
              beatmap.id
            }?n=1)`
          )
          .addField(
            `${starrate} __${beatmap.version}__`,
            `▸**Difficulty:** ${diffi}★ ▸**Max Combo:** ${beatmap.maxCombo}\n▸**AR:** ${beatmap.difficulty.approach} ▸**OD:** ${beatmap.difficulty.overall} ▸**CS:** ${beatmap.difficulty.size} ▸**HP:** ${beatmap.difficulty.drain}\n▸**PP:** ▫ **95%**- ▫ **99%**- ▫ **100%**-`
          )
          .setFooter(
            `${beatmap.approvalStatus} | ${beatmap.counts.favorites}❤ | Approved ${beatmap.raw_approvedDate}`
          );
    ...
    if (args[0] == "user") {
      ...
      osuapi.getUser({ u: username }).then((user) => {
        ...
        const embed = new discord.MessageEmbed()
          ...
          .setDescription(
            `▸**Bancho Rank:** #${Intl.NumberFormat().format(
              user.pp.rank
            )} (#${Intl.NumberFormat().format(user.pp.countryRank)})\n`
          )
          .setThumbnail(pfp)
          .setColor("#8427EE");
        message.channel.send(embed);
