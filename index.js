...
client.on("message", function (message) {
  if (message.content === "mến") {
    message.channel.send(
      "https://media.discordapp.net/attachments/899238558185959505/911258764148097024/Z.png"
    );
  }
});
const rando_imgs = [
...
client.on("message", (message) => {
  if (message.content.startsWith("sh!botannsfw")) {
    ...
    message.channel
      .send("***My waifu nsfw***", { files: [response] })
      .then((msg) => {
  ...
  client.channels.cache
    ...
    .send(
      " ✅ " +
        ...
        `**${message.content}**` +
        " in " +
        `${message.channel} (${message.guild.name})`
    );
  console.log(
    ` ✅ ${chalk.greenBright.bold(message.author.tag)} ${chalk.cyan(
      "executed"
    )} ${chalk.greenBright.bold(message.content)} ${chalk.cyan(
      "in"
    )} ${chalk.magenta.bold(message.channel.name)} ${chalk.magenta.bold(
      `(${message.guild.name})`
    )}`
  );
});
...
client.on("message", (message) => {
  ...
  if (character) {
    ...
    require("request")(
      `https://danbooru.donmai.us/posts.json?limit=1&tags=${character}+order%3Arandom` +
        "+" +
        newargs,
      function (res, err, body) {
        ...
        if (data[0] !== undefined) {
          ...
          const embed = new discord.MessageEmbed()
            ...
            .addField(
              "Dimension:",
              data[0].image_width + "x" + data[0].image_height,
              true
            )
            .addField("Is NSFW:", nsfw, true)
            .addField(
              "Created at:",
              pTime.substr(0, 10).replace(/-/g, "/"),
              true
            )
...
client.on('guildCreate', async(guild) => {
    const channel = '923396856531275786'
    const ownerID = '607086287467577344'
    const embed = new MessageEmbed()
    .setAuthor(`New guild joined`)
    .setColor('#35D8BD')
    .setThumbnail(guild.iconURL())
    .setDescription(`**Guild Name:** ${guild.name}\n**Guild ID:** ${guild.id}\n \n**Created at:** ${guild.createdAt.toLocaleDateString('vi-vn')}\n**Guild Owner:** ${guild.owner.user.username}#${guild.owner.user.discriminator}`)
    client.channels.cache.get(`${channel}`).send(`<@${ownerID}>, Your bot has been invited to a new server`, embed)
})
